...
Similarly, I was easily able to download the monthly precipitation data in 5 year increments from the California Department of Water Resources website. I used to `rbind()` function to combine three datasets into one dataframe that includede data from 2003-2019. From this dataframe I was able to calculate the annual average precipitation in centimeters. I then performed an STL decomposition so I could see any trends in the precipitation data.
...
![\[Figure 2\] The left figure shows the change in area of cultivated crop lands from 2003-2021 in square meters. The right figure shows the STL decomposition of the annual average water equivalent thickness anomaly(cm) from 2002-2019.](trends.jpeg)
...
![\[Figure 3\] This figrue shows the comparison of two STL decompositions. Thee left three graphs show the STL decomposition for the Annual Average Precipitation(centimeters) measured byt eh Woodland Weather Station. The right three graphs shows the STL decomposition of the annual average water equivalent thickness anomaly(cm) from 2002-2019.](Decomp.png)
...
The next set of linear regressions used polynomials for the cultivated crop variable. I ran two experiments looking at a quadratic and cubic equation for cultivated crop land.

In order to test for ommittted variables bias, I tried to improve my model by running two multiple linear regression models that used changes in developed area and annual average precipitation as the additional variables in the multiple linear regression model.

## Results

After running a variety a linear regressions I found that the model with the best fit for my data was a multiple linear regression model with cultivated crop area and annual average precipitation as the variables. Figure 4 shows a summary of the linear regression output.
...
It is worth noting that the Adjusted R-squared value is 0.4972, which implies that 49% of the variation in my model could be explained by just cultivated crop land use change and annual precipitation.

Based on the results of my summary above, I was not able to reject the null hypothesis at a significance level of 0.5. While I was not able to reject the null hypothesis, these p-values are not so high that they imply there in no relationship between these variables.

Since the multiple linear regression model was the best fit for the data, it implies that there was omitted variables bias in my other regressions. This makes sense because the total water storage is likely affected by many factors that were not accounted for in this short analysis.
